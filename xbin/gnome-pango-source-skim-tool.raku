...
#  $section-doc = get-section($source-content);
  ...
  self._set-class-name-of-sub('GSimpleAction') if ?$s;
      ...
      $template-text = Q:q:to/EOTEMPLATE/;
        ...
        submethod BUILD ( *%options ) {
          ...
          if self.^name eq 'Gnome::LIBRARYMODULE' {
...
#-------------------------------------------------------------------------------
# NOTE;  this documentation is different from Gtk3. Looks like Gtk4 btw.

# get the class title and class info from the source file
sub get-section ( Str:D $source-content is rw --> Str ) {

  return Str unless $source-content;

  # Doc is at the top, so should not give false positives
  $source-content ~~ m/ '/**' .*? ':' .*? '*/' /;
  return Str unless ?$/;

  # get the doc and remove from $source-content
  my Str $section-doc = ~$/;
  $source-content ~~ s/$section-doc//;


#`{{ There is no title nore see also
...
#-------------------------------------------------------------------------------
sub get-signals ( Str:D $source-content is rw ) {
  return unless $source-content;
